...
console.log(
  `Elliptic curve y^2 = x^3 + ${a}x + ${b} defined over \u1d53d ${p} (ℤ/${p}ℤ) with order ${E.length} \n`
);
...
const groupAction = ([Xq, Yq], [Xp, Yp]) => {
  ...
  if (Xp == Xq) {
    // either P and Q are opposite eachother or they're the same point

    if (Yp == mod(-Yq, p)) {
      // if their y-coordinates are inverses (refl across y-axis)

      return [Infinity, Infinity]; // return the identity point
    } else {
      // otherwise, they must be exactly equivalant
    ...
    // this is the normal case; P =/= Q,  y_p =/= -y_q mod p
...
const repeatedAction = ([a, b], times) => {
  // sexy little self-referential algorithm here
  if (times === 1) {
    return [a, b];
  } else if (times > 1) {
...
if (subgroup.length == E.length) {
  console.log(
    "elliptic curve is cyclic and thus isomorphic to the additive group of the finite field"
  );
