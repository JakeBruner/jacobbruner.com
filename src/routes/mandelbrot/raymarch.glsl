...
Dual mult(Dual a, Dual b) {
  return {
  a.val * b.val, a.der * b.val + a.val * b.der };
}

// Function to compute the distance to the fractal surface
// Roughly speaking DE = (r- escapeRadius)/dr
// Using dual numbers to speed up operation
Dual fractalDF(Dual position) {
Dual z = position;
Dual dr = {
1.0, 0.0 };
float r = 0.0;
...
float r7 = pow(r, 7.0);
float r8 = pow(r, 8.0);
float theta = acos(z.val.z / r);
float phi = atan(z.val.y, z.val.x);
...
r8, 0.0 }
, {
vec3(sin(theta) * cos(phi), sin(phi) * sin(theta), cos(theta)), vec3(0.0, 0.0, 0.0) }
);
z.val += position.val;
z.der += position.der;
}
return {
0.5 * log(r) * r / dr.val, (0.5 * r / (dr.val * dr.val)) * (1 + log(r)) - 0.5 * log(r) };
}

Dual lowp_fractalDF(Dual position) {
Dual z = position;
Dual dr = {
1.0, 0.0 };
float r = 0.0;
...
vec3 x = fractalDF({
position + vec3(EPS, 0.0, 0.0), vec3(1.0, 0.0, 0.0) }
).der;
vec3 y = fractalDF({
position + vec3(0.0, EPS, 0.0), vec3(0.0, 1.0, 0.0) }
).der;
vec3 z = fractalDF({
position + vec3(0.0, 0.0, EPS), vec3(0.0, 0.0, 1.0) }
).der;
...
col = vec3(0.54f, 0.3f, 0.6) - (rayDir.y * 0.4);
